...
import {FileExplorer, FileExplorerProps} from "@/components/azshoyos/Program/FileExplorer";
import {DocumentReader, DocumentReaderProps} from "@/components/azshoyos/Program/DocumentReader";
import {Shortcut} from "@/components/azshoyos/Shortcut/Shortcut";
import {ShellConsole, ShellConsoleProps} from "@/components/azshoyos/Program/Console";
import {v2} from "@/util/types";
import {quickPaths} from "@/components/azshoyos/ProgramData/programs";
import {AppWindow, AppWindowProps} from "@/components/azshoyos/Program/AppWindow";
...
export class AppWindowProgramClass extends ProgramBase {
  programType: ProgramType = ProgramType.APP_WINDOW
  windowComponent = AppWindow
  parameters: AppWindowProps
  constructor(construct:ProgramBaseProps & AppWindowProps) {
    super(construct)
    this.parameters = {
      appId: construct.appId,
    }
  }
  run = () => {
    return this.windowComponent(this.parameters)
...
export type WindowedProgramClass = FileExplorerProgramClass | DocumentReaderProgramClass | ShellConsoleProgramClass | AppWindowProgramClass
...
export type ProgramConstructor =
  ...
  ({type: ProgramType.APP_WINDOW} & BaseWithoutID & AppWindowProps)

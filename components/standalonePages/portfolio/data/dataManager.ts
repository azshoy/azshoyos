...
export const getKeyWords = (p:ListItem) => {
  ...
  Object.keys(p.tags).forEach((tg) => {
    getNonConflictingWords(pwordsLow, pwords, p.tags[tg])
  })
...
const getNonConflictingWords = (a: string[], l: string[], b:string | string[] | undefined, allLower=false) => {
  if (!b) return
  if (typeof b !== "string") {
    b.forEach((bs) => {
      getNonConflictingWords(a, l, bs)
    })
    return
  }
...
export const getKeywordOptionsForSearch = (dict: ItemDict, preList: string[] = []) => {
  ...
  Object.values(dict).forEach((li) => {
    Object.keys(li.tags).forEach((tg) => {
      getNonConflictingWords(unique, keywords, li.tags[tg])
    })
